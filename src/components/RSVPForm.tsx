...
import { User, Users, Check, AlertCircle, Sparkles, Music } from 'lucide-react';
import { RSVP } from '../types';

const SHEETS_WEBAPP_URL =
   (import.meta as any).env?.VITE_GOOGLE_SHEETS_WEBAPP_URL || '';
...
export default function RSVPForm() {
   ...
   const [rsvpId] = useState<string | null>(null);
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      if (formData.attendance === 'yes') {
         if (
            !formData.arrivalLocations ||
            formData.arrivalLocations.length === 0
         ) {
            setErrorMsg(
               'Խնդրում ենք ընտրել առնվազն մեկ արարողություն, որին մասնակցելու եք։',
            );
            setLoading(false);
            return;
         }
      ...
      try {
         ...
         try {
            ...
            console.warn(
               'Local API endpoint not available, falling back to direct Sheets client-side fetch...',
               err,
            );
         }
         ...
         if (!ok && !triedLocal && SHEETS_WEBAPP_URL) {
            ...
            if (res.ok) {
               ...
               if (json.status === 'error') {
                  setErrorMsg(
                     json.error || 'Google Sheets script returned an error.',
                  );
               } else {
                  ok = true;
                  result = {
               ...
               setErrorMsg(
                  'Could not store RSVP to Google Sheets Web App. Please check the network.',
               );
            }
         }
   ...
   return (
      <section
         id="rsvp-section"
         className="py-16 px-4 bg-[#faf6f0] border-t border-[#ece4db] select-none">
                  ...
                  {success ? (
                     ...
                     <motion.form
                        ...
                        <div className="space-y-1.5">
                           <label
                              htmlFor="name"
                              className="block text-xs font-semibold uppercase tracking-wide text-[#8a7261]">
                              ...
                              {[
                                 {
                                    val: 'yes',
                                    label: 'Սիրով կգամ',
                                    bg: 'hover:border-[#bf9c85]',
                                    activeBg:
                                       'bg-[#bf9c85] text-white border-[#bf9c85]',
                                 },
                                 {
                                    val: 'no',
                                    label: 'Ցավոք, չեմ կարող',
                                    bg: 'hover:border-[#8d705c]',
                                    activeBg:
                                       'bg-[#8d705c] text-white border-[#8d705c]',
                                 },
                              ].map((btn) => (
                                 <button
                                    key={btn.val}
                        ...
                        {formData.attendance === 'yes' && (
                           <div className="space-y-4 pt-2">
                              ...
                              <div className="space-y-2 pt-1">
                                 <label className="block text-xs font-semibold uppercase tracking-wide text-[#8a7261]">
                                    Ո՞ր արարողություններին եք մասնակցելու{' '}
                                    <span className="text-[#bf9c85] font-extrabold">
                                       *
                                    </span>
                                 </label>
                                 <div className="space-y-2">
                                    {[
                                       {
                                          id: 'prep',
                                          label: 'Հարսի տուն՝ ք․ Երևան',
                                          desc: 'Ավանդական առավոտյան հանդիսություն և հարսի տան արարողություն',
                                       },
                                       {
                                          id: 'church',
                                          label: 'Պսակադրություն',
                                          desc: 'Պսակադրություն և սուրբ ուխտի օրհնություն Նորք-Մարաշի Սուրբ Աստվածածին եկեղեցում',
                                       },
                                       {
                                          id: 'restaurant',
                                          label: 'Հարսանեկան Խնջույք',
                                          desc: 'Հարսանեկան խնջույք և ընթրիք «Ոսկե Ծիրան» այգի-ռեստորանում',
                                       },
                                    ].map((loc) => {
                                       const isSelected = (
                                          formData.arrivalLocations || []
                                       ...
                                       return (
                                          <button
                                             ...
                                             type="button"
                                             onClick={() =>
                                                handleLocationToggle(loc.label)
                                             }
